...
function FiberNode(
  ...
  // Instance
  this.tag = tag; // 代表fiber节点的类型，fiber节点类型可以是：hostRoot（表示宿主环境下的根节点，react不仅仅可以在浏览器中运行，也可以在app中运行），对应的rootFiber；functionComponent；classComponent
  this.key = key;
  this.elementType = null; // React内部定制的，ReactElement的类型
  this.type = null; // 代表faiber节点对应的虚拟DOM节点的类型，也就是createElement的第一个参数
  this.stateNode = null; // fiber对应的真实DOM节点，暂时可以这样理解（函数组件、类组件没有真实的DOM节点）

  // Fiber
  this.return = null; // 指向父节点
  this.child = null;  // 指向第一个子节点
  this.sibling = null; // 指向兄弟节点
  this.index = 0; // fiber在兄弟节点之间的索引位置，也就是在同级的位置

  this.ref = null;

  /* 储存状态与hook */
  this.pendingProps = pendingProps; // 等待生效的props，也就是刚刚传过来的props
  ...
  // 更新类型
  this.flags = NoFlags; // 用来标记节点的更新类型，如果没有更新的就是NoFlags
  ...
  this.alternate = null; // 轮替，current fiber树的节点和workInProgress fiber树的节点之间相互引用的属性
